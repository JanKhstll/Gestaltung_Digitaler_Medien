...
<html>
 <head>
  ...
  <title>kaufalbum</title>
  <!-- CSS -->
  <link rel="stylesheet" type="text/css" href="css/site_global.css"/>
  <link rel="stylesheet" type="text/css" href="css/kaufalbum.css"/>
  <!-- Other scripts -->
...
<!--Adobe Edge Runtime-->
    ...
    <script type="text/javascript" charset="utf-8" src="Alben_edge.js"></script>
    <script type="text/javascript" charset="utf-8" src="Alben_edgeActions.js"></script>
    <link rel="stylesheet" href="Alben_edge.css" />
<!--Adobe Edge Runtime End-->
   ...
   $suche = $_POST["Albumauswahl"]  ;

echo "<html>

  <head>
  <title>Bestellung eintragen</title>
  </head>

  <body>";
    include("db.php");

   $res = mysql_query("SELECT albname,albpreis FROM tblalbum
                                   WHERE albnr =\"$suche\"");
  $name = mysql_result($res, 0, "albname");
  $preis = mysql_result($res, 0, "albpreis");


 echo '<form name="Auswahl_Interpret" action="loginalbum.php" method="POST" enctype="text/html">  '   ;

echo "<h1>$name</h1><br />" ;
echo "Dieses Album kostet $preis Euro. Unten sehen Sie alle Lieder des Albums aufgelistet. <br /><br />" ;


echo "<table border='0' cellpadding='2' cellspacing='2' summary='' bgcolor='#d2d0d0'>
    <tr>
         <td style='width: 215px;'><b>Titel</b></td>
        <td style='width: 115px;'><b>Interpret</b></td>
        <td style='width: 115px;'><b>Genre</b></td>
    </tr>";

  $sql = "SELECT * FROM `tbllied` LEFT JOIN (tblinterpret,tblgenre) ON
(tbllied.InterNr=tblinterpret.InterNr AND tbllied.GenreNr=tblgenre.GenreNr) WHERE `albnr` LIKE '$suche'  ";
...
<table border="0" cellpadding="2" cellspacing="2" summary="">
    <tr>
              ...
              <td style="width: 215px;"><?PHP echo $line['LiedName'];?></td>
...
<input type="hidden" name="Albumkauf"
