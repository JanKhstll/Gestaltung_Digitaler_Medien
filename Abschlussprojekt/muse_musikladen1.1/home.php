<!DOCTYPE html>
<html>
 <head>

  <meta http-equiv="Content-type" content="text/html;charset=UTF-8"/>
  <meta name="generator" content="0.8.755.98"/>
  <title>Home</title>
  <!-- CSS -->
  <link rel="stylesheet" type="text/css" href="css/site_global.css"/>
  <link rel="stylesheet" type="text/css" href="css/index.css"/>
  <!-- Other scripts -->
  <script type="text/javascript">
   document.documentElement.className = 'js';
</script>
   </head>
 <body>

  <div class="clearfix" id="page"><!-- column -->
   <div class="position_content" id="page_position_content">
    <div class="clearfix colelem" id="n1"><!-- group -->
     <div class="clearfix grpelem" id="n2"><!-- group -->
      <ul class="MenuBar clearfix grpelem" id="menu"><!-- row -->
       <li class="MenuItemContainer clearfix grpelem" id="n3"><!-- vertical box -->
        <a class="nonblock nontext MuseLinkActive MenuItem MenuItemWithSubMenu MuseMenuActive clearfix colelem" id="n4" href="home.php"><!-- state-based group --><div class="grpelem" id="n5"><!-- horizontal box --><div class="_n5 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n6"><!-- content --><p>Home</p></div></div></div><div class="grpelem" id="n10"><!-- horizontal box --><div class="_n10 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n11"><!-- content --><p>Home</p></div></div></div><div class="grpelem" id="n15"><!-- horizontal box --><div class="_n15 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n16"><!-- content --><p>Home</p></div></div></div><div class="grpelem" id="n20"><!-- horizontal box --><div class="_n20 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n21"><!-- content --><p>Home</p></div></div></div></a>
       </li>
       <li class="MenuItemContainer clearfix grpelem" id="n25"><!-- vertical box -->
        <a class="nonblock nontext MenuItem MenuItemWithSubMenu clearfix colelem" id="n26" href="interpret.php"><!-- state-based group --><div class="grpelem" id="n27"><!-- horizontal box --><div class="_n27 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n28"><!-- content --><p>Interpret</p></div></div></div><div class="grpelem" id="n32"><!-- horizontal box --><div class="_n32 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n33"><!-- content --><p>Interpret</p></div></div></div><div class="grpelem" id="n37"><!-- horizontal box --><div class="_n37 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n38"><!-- content --><p>Interpret</p></div></div></div><div class="grpelem" id="n42"><!-- horizontal box --><div class="_n42 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n43"><!-- content --><p>Interpret</p></div></div></div></a>
       </li>
       <li class="MenuItemContainer clearfix grpelem" id="n47"><!-- vertical box -->
        <a class="nonblock nontext MenuItem MenuItemWithSubMenu clearfix colelem" id="n48" href="album.php"><!-- state-based group --><div class="grpelem" id="n49"><!-- horizontal box --><div class="_n49 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n50"><!-- content --><p>Album</p></div></div></div><div class="grpelem" id="n54"><!-- horizontal box --><div class="_n54 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n55"><!-- content --><p>Album</p></div></div></div><div class="grpelem" id="n59"><!-- horizontal box --><div class="_n59 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n60"><!-- content --><p>Album</p></div></div></div><div class="grpelem" id="n64"><!-- horizontal box --><div class="_n64 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n65"><!-- content --><p>Album</p></div></div></div></a>
       </li>
       <li class="MenuItemContainer clearfix grpelem" id="n69"><!-- vertical box -->
        <a class="nonblock nontext MenuItem MenuItemWithSubMenu clearfix colelem" id="n70" href="genres.php"><!-- state-based group --><div class="grpelem" id="n71"><!-- horizontal box --><div class="_n71 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n72"><!-- content --><p>Genres</p></div></div></div><div class="grpelem" id="n76"><!-- horizontal box --><div class="_n76 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n77"><!-- content --><p>Genres</p></div></div></div><div class="grpelem" id="n81"><!-- horizontal box --><div class="_n81 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n82"><!-- content --><p>Genres</p></div></div></div><div class="grpelem" id="n86"><!-- horizontal box --><div class="_n86 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n87"><!-- content --><p>Genres</p></div></div></div></a>
       </li>
       <li class="MenuItemContainer clearfix grpelem" id="n91"><!-- vertical box -->
        <a class="nonblock nontext MenuItem MenuItemWithSubMenu clearfix colelem" id="n92" href="charts.php"><!-- state-based group --><div class="grpelem" id="n93"><!-- horizontal box --><div class="_n93 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n94"><!-- content --><p>Charts</p></div></div></div><div class="grpelem" id="n98"><!-- horizontal box --><div class="_n98 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n99"><!-- content --><p>Charts</p></div></div></div><div class="grpelem" id="n103"><!-- horizontal box --><div class="_n103 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n104"><!-- content --><p>Charts</p></div></div></div><div class="grpelem" id="n108"><!-- horizontal box --><div class="_n108 f3s_top clearfix"><div class="MenuItemLabel NoWrap clearfix grpelem" id="n109"><!-- content --><p>Charts</p></div></div></div></a>
       </li>
      </ul>
     </div>
     <div class="clearfix grpelem" id="n113"><!-- column -->
      <div class="colelem" id="n114"><!-- custom html -->
       <!DOCTYPE html>
<html>
<head>
	<title>Untitled</title>
<!--Adobe Edge Runtime-->
    <script type="text/javascript" src="edge_includes/jquery-1.6.2.min.js"></script>
    <script type="text/javascript" src="edge_includes/jquery.easing.1.3.js"></script>
    <script type="text/javascript" src="edge_includes/edge.0.1.3.min.js"></script>
    <script type="text/javascript" src="edge_includes/edge.symbol.0.1.3.min.js"></script>
    <script type="text/javascript" charset="utf-8" src="Home_edge.js"></script>
    <script type="text/javascript" charset="utf-8" src="Home_edgeActions.js"></script>
    <link rel="stylesheet" href="Home_edge.css" />
<!--Adobe Edge Runtime End-->

</head><body style="margin:0;padding:0;">
	<div id="stage" class="EDGE-3735892">
	</div>
</body>
</html>

</div>
      <div class="clearfix colelem" id="n115"><!-- group -->
       <div class="clearfix grpelem" id="n116"><!-- column -->
        <div class="clearfix colelem" id="n117"><!-- group -->
         <div class="clearfix grpelem" id="n118"><!-- content -->
          <p id="n120"><span id="n121">Willkommen auf unserer Homepage!</span><span> </span></p>
         </div>
         <div class="grpelem" id="n123"><!-- custom html -->
          <form name="Suche" action="Suchausgabe.php" method="POST" enctype="text/html">
<input type="text" name="Suchbegriff" value="" size="20" maxlength="50" style="border: #d2d0d0;
       border-style: solid; border-top-width: 2px; border-right-width: 2px;
       border-bottom-width: 2px; border-left-width: 2px" />
<input type="submit" name="" value="Suchen" />
</form>

</div>
        </div>
        <div class="SlideShowWidget clearfix colelem" id="slideshow"><!-- group -->
         <div class="popup_anchor">
          <div class="SlideShowContentPanel clearfix" id="n124"><!-- stack box -->
           <div class="SSSlide grpelem" id="n125"><!-- image -->
            <img id="n125_img" src="image/rihanna%20dior%20at%20grammy20113.jpg" alt="" width="384" height="216"/>
           </div>
           <div class="SSSlide invi grpelem" id="n127"><!-- image -->
            <img id="n127_img" src="image/112211-blogs-jennifer-lopez-and-casper.jpg" alt="" width="400" height="249"/>
           </div>
           <div class="SSSlide invi grpelem" id="n129"><!-- image -->
            <img id="n129_img" src="image/beyonce-turneu2.jpg" alt="" width="384" height="256"/>
           </div>
           <div class="SSSlide invi grpelem" id="n131"><!-- image -->
            <img id="n131_img" src="image/index_n131.jpg" alt="" width="384" height="275"/>
           </div>
           <div class="SSSlide invi grpelem" id="n133"><!-- image -->
            <img id="n133_img" src="image/img_7539.jpg" alt="" width="367" height="275"/>
           </div>
           <div class="SSSlide invi grpelem" id="n135"><!-- image -->
            <img id="n135_img" src="image/img_2453.jpg" alt="" width="367" height="275"/>
           </div>
           <div class="SSSlide invi grpelem" id="n137"><!-- image -->
            <img id="n137_img" src="image/rihanna-nba-all-star.jpg" alt="" width="400" height="267"/>
           </div>
          </div>
         </div>
         <div class="popup_anchor">
          <div class="SlideShowCaptionPanel clearfix" id="n139"><!-- stack box -->
           <div class="SSSlideCaption clearfix grpelem" id="n140"><!-- content -->
            <p>[Rihanna]</p>
           </div>
           <div class="SSSlideCaption invi clearfix grpelem" id="n144"><!-- content -->
            <p>[Jennifer Lopez]</p>
           </div>
           <div class="SSSlideCaption invi clearfix grpelem" id="n148"><!-- content -->
            <p>[Beyoncé]</p>
           </div>
           <div class="SSSlideCaption invi clearfix grpelem" id="n152"><!-- content -->
            <p>[Adele]</p>
           </div>
           <div class="SSSlideCaption invi clearfix grpelem" id="n156"><!-- content -->
            <p>[Coldplay]</p>
           </div>
           <div class="SSSlideCaption invi clearfix grpelem" id="n160"><!-- content -->
            <p>&nbsp; </p>
           </div>
           <div class="SSSlideCaption invi clearfix grpelem" id="n163"><!-- content -->
            <p>[Rihanna]</p>
           </div>
          </div>
         </div>
         <div class="popup_anchor">
          <div class="SSPreviousButton clearfix" id="n167"><!-- content -->
           <p>&lt;</p>
          </div>
         </div>
         <div class="popup_anchor">
          <div class="SSNextButton clearfix" id="n171"><!-- content -->
           <p>&gt;</p>
          </div>
         </div>
         <div class="popup_anchor">
          <div class="SlideShowLabel SSSlideCount clearfix" id="n175"><!-- content -->
           <p>7 &#45; 7</p>
          </div>
         </div>
         <div class="popup_anchor">
          <div class="SSSlideLinks clearfix" id="n179"><!-- horizontal-rows box -->
           <div class="SSSlideLink grpelem" id="n180"><!-- image -->
            <img id="n180_img" src="image/index_n180.jpg" alt="" width="59" height="42"/>
           </div>
           <div class="SSSlideLink grpelem" id="n182"><!-- image -->
            <img id="n182_img" src="image/index_n182.jpg" alt="" width="59" height="42"/>
           </div>
           <div class="SSSlideLink grpelem" id="n184"><!-- image -->
            <img id="n184_img" src="image/index_n184.jpg" alt="" width="59" height="42"/>
           </div>
           <div class="SSSlideLink grpelem" id="n186"><!-- image -->
            <img id="n186_img" src="image/index_n186.jpg" alt="" width="59" height="42"/>
           </div>
           <div class="SSSlideLink grpelem" id="n188"><!-- image -->
            <img id="n188_img" src="image/index_n188.jpg" alt="" width="59" height="42"/>
           </div>
           <div class="SSSlideLink grpelem" id="n190"><!-- image -->
            <img id="n190_img" src="image/index_n190.jpg" alt="" width="59" height="42"/>
           </div>
           <div class="SSSlideLink grpelem" id="n192"><!-- image -->
            <img id="n192_img" src="image/index_n192.jpg" alt="" width="59" height="42"/>
           </div>
          </div>
         </div>
        </div>
       </div>
       <div class="grpelem" id="n194"><!-- custom html -->
        <?php

 if (isset($_COOKIE['KuNr'], $_COOKIE['Password'])) {

echo '<br /><br />Sie wollen sich abmelden? <a href="logout.php">HIER AUSLOGGEN</a> <br />';
echo ' <a href="konto.php">Konto bearbeiten</a> <br />';


  }

  else
  {
 echo   ' <br /><br />Noch kein Kunde? <a href="Registrierung.php">HIER REGISTRIEREN</a>
<br /><br />Schon Kunde? <a href="login_neu.php">HIER EINLOGGEN</a>' ;
  }


 if (isset($_COOKIE['KuNr']))
 {


 $KuNr = $_COOKIE['KuNr']  ;

     if ($KuNr ==  '21')
 {
     echo  '
    <a href="Liedadd.php">Lied hinzuf&uuml;gen</a>                               <br />
<a href="interadd.php">Interpret hinzuf&uuml;gen</a>                                     <br />
<a href="albumadd.php">Album hinzuf&uuml;gen</a>                                     <br />

     ' ;
 }

}


?>

</div>
      </div>
     </div>
    </div>
    <div class="clearfix colelem" id="master-footer"><!-- group -->
     <div class="clearfix grpelem" id="n195"><!-- content -->
      <p>2012 © Jan Kohstall &amp; Julian Kappler </p>
     </div>
    </div>
   </div>
  </div>
  <div class="preload_images">
   <img class="preload" src="image/master_u425_r_full.png" alt=""/>
   <img class="preload" src="image/master_u425_m_full.png" alt=""/>
   <img class="preload" src="image/master_u425_a_full.png" alt=""/>
   <img class="preload" src="image/master_u404_r_full.png" alt=""/>
   <img class="preload" src="image/master_u404_m_full.png" alt=""/>
   <img class="preload" src="image/master_u404_a_full.png" alt=""/>
   <img class="preload" src="image/master_u416_r_full.png" alt=""/>
   <img class="preload" src="image/master_u416_m_full.png" alt=""/>
   <img class="preload" src="image/master_u416_a_full.png" alt=""/>
   <img class="preload" src="image/master_u430_r_full.png" alt=""/>
   <img class="preload" src="image/master_u430_m_full.png" alt=""/>
   <img class="preload" src="image/master_u430_a_full.png" alt=""/>
   <img class="preload" src="image/master_u411_r_full.png" alt=""/>
   <img class="preload" src="image/master_u411_m_full.png" alt=""/>
   <img class="preload" src="image/master_u411_a_full.png" alt=""/>
  </div>
  <!-- JS includes -->
 
  <script type="text/javascript">
   window.jQuery || document.write('\x3Cscript src="scripts/1.1/jquery-1.7.min.js" type="text/javascript">\x3C/script>');
</script>
  <script src="scripts/1.1/sprydomutils.js" type="text/javascript"></script>
  <script src="scripts/1.1/museutils.js" type="text/javascript"></script>
  <script src="scripts/1.1/jquery.musemenu.js" type="text/javascript"></script>
  <script src="scripts/1.1/musesprycommon.js" type="text/javascript"></script>
  <script src="scripts/1.1/musesprypanels.js" type="text/javascript"></script>
  <script src="scripts/1.1/musespryslideshow.js" type="text/javascript"></script>
  <script src="scripts/1.1/jquery.museoverlay.js" type="text/javascript"></script>
  <!-- Other scripts -->
  <script type="text/javascript">
   Muse.Utils.addSelectorFn('body', Muse.Utils.transformMarkupToFixBrowserProblemsPreInit);/* body */
Muse.Utils.addSelectorFn('a.nonblock', Muse.Utils.addHyperlinkAnchor); /* a.nonblock */
Muse.Utils.addSelectorFn('.MenuBar', function(elem) { return $(elem).museMenu(); });/* unifiedNavBar */
Muse.Utils.addSelectorFn('#slideshow', function(elem) { new Spry.Widget.ContentSlideShow(elem, {autoPlay:true,displayInterval:3000,slideLinkStopsSlideShow:false,transitionStyle:'fading',plugIns:[],lightboxEnabled_runtime:false}); });/* #slideshow */
Muse.Utils.addSelectorFn('body', Muse.Utils.showWidgetsWhenReady);/* body */
Muse.Utils.addSelectorFn('body', Muse.Utils.transformMarkupToFixBrowserProblems);/* body */

</script>
   </body>
</html>